...
const Bulletin = () => {
  return (
    <div className='bulletin'>
      <h1 className="heading">Opportunities in the Spotlight!</h1>
      <div className="container">
        {bulletins.map((bulletin) => (
          <div key={bulletin.key} className="note">
            <h2>{bulletin.title}</h2>
            <p>{bulletin.content}</p>
            <p className="update-date">{bulletin.updateDate}</p>
            <a href={bulletin.link} className="link" target="_blank" rel="noopener noreferrer">Read more</a> {/* Link to the full article */}
          </div>
        ))}
      </div>
    </div>
